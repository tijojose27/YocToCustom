...
package com.example.yoctocustom.Yocto;
    ...
    public int _loadAngles() throws YAPI_Exception
    {
        ...
        if (_angles_stamp != _qt_stamp) {
            ...
            if (delta > 0.499 * norm) {
                ...
                _pitch = 90.0;
                _head  = (double)Math.round(2.0 * 1800.0/ Math.PI * Math.atan2(_x,-_w)) / 10.0;
            } else {
                if (delta < -0.499 * norm) {
                    ...
                    _roll  = (double)Math.round(1800.0/ Math.PI * Math.atan2(2.0 * (_w * _x + _y * _z),sqw - sqx - sqy + sqz)) / 10.0;
                    _pitch = (double)Math.round(1800.0/ Math.PI * Math.asin(2.0 * delta / norm)) / 10.0;
                    _head  = (double)Math.round(1800.0/ Math.PI * Math.atan2(2.0 * (_x * _y + _z * _w),sqw + sqx - sqy - sqz)) / 10.0;
                }
            }
